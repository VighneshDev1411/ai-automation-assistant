...
export function WorkflowBuilder({
  ...
  const getDefaultConfig = (nodeType: string) => {
    switch (nodeType) {
      ...
      case 'action':
        return { type: 'http', config: { url: '', method: 'GET' } }
      case 'condition':
        return { field: '', operator: 'equals', value: '' }
      case 'transform':
        return { type: 'map', mapping: {} }
      default:
        return {}
    }
  }
...
function NodeInspector({ node, onUpdate, onDelete, onClose }: NodeInspectorProps) {
  ...
  return (
    <Card className="absolute top-4 right-4 w-80 max-h-96 overflow-y-auto z-10">
...
function ActionNodeConfig({ config, onUpdate }: { config: any; onUpdate: (config: any) => void }) {
  return (
    <div className="space-y-3">
      <div>
